#use drop list updated on last sat and mon
#changed file and worksheet names to find to 10 days ago (instead of 7)
#may need to change file and ws2 variable to correct name of file and worksheet
...
prevmon = date.today()- timedelta(days=10)
os.chdir('S:\\shares\\Pathology\\documents\\PATH-LABS\\CYTOGENETICS\\CYTOGEN\\Daily Log\\New DropLists')
file = "ChromosomeCases Week of_" + str(date.today()- timedelta(days=10)) + ".xlsx"
...
for index, row in enumerate(ws2.iter_rows()):
    ...
    try:
        if "S19" in row[0].value or "CG19" in row[0].value or "FL19" in row[0].value:
            ...
            if datetime.date.today()-timedelta(days=5) == ws2.cell(row=index+2, column=5).value.date() or datetime.date.today()-timedelta(days=3) == ws2.cell(row=index+2, column=5).value.date():
                #print("hi")
            ...
            if datetime.date.today()-timedelta(days=5) == ws2.cell(row=index+2, column=6).value.date() or datetime.date.today()-timedelta(days=3) == ws2.cell(row=index+2, column=6).value.date():
                #print("no")
                ws.cell(column=3, row=new_line_num, value=row[0].value)
                ws.cell(column=4, row=new_line_num, value=row[1].value)
                ws.cell(column=5, row=new_line_num, value=row[2].value)
            ...
            if datetime.date.today()-timedelta(days=5) == ws2.cell(row=index+2, column=7).value.date() or datetime.date.today()-timedelta(days=3) == ws2.cell(row=index+2, column=7).value.date():
                ws.cell(column=3, row=new_line_num, value=row[0].value)
                ws.cell(column=4, row=new_line_num, value=row[1].value)
                ws.cell(column=5, row=new_line_num, value=row[2].value)
                ws.cell(column=8, row=new_line_num, value=row[6].value)
                print(row[0].value + " " + row[6].value)
            if datetime.date.today()-timedelta(days=5) == ws2.cell(row=index+2, column=8).value.date() or datetime.date.today()-timedelta(days=3) == ws2.cell(row=index+2, column=8).value.date():    
                ws.cell(column=3, row=new_line_num, value=row[0].value)
                ws.cell(column=4, row=new_line_num, value=row[1].value)
                ws.cell(column=5, row=new_line_num, value=row[2].value)
                ws.cell(column=9, row=new_line_num, value=row[7].value)
                print(row[0].value + " " + row[7].value)
            if datetime.date.today()-timedelta(days=5) == ws2.cell(row=index+2, column=9).value.date() or datetime.date.today()-timedelta(days=3) == ws2.cell(row=index+2, column=9).value.date():
                ws.cell(column=3, row=new_line_num, value=row[0].value)
                ws.cell(column=4, row=new_line_num, value=row[1].value)
                ws.cell(column=5, row=new_line_num, value=row[2].value)
                ws.cell(column=10, row=new_line_num, value=row[8].value)
                print(row[0].value + " " + row[8].value)
            if datetime.date.today()-timedelta(days=5) == ws2.cell(row=index+2, column=10).value.date() or datetime.date.today()-timedelta(days=3) == ws2.cell(row=index+2, column=10).value.date():
                ws.cell(column=3, row=new_line_num, value=row[0].value)
                ws.cell(column=4, row=new_line_num, value=row[1].value)
                ws.cell(column=5, row=new_line_num, value=row[2].value)
