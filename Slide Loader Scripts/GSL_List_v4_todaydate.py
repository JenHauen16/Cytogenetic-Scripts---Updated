...
wb = Workbook()
wsname = "GSL Cases for " + str(date.today()+ timedelta(days=1))
...
#file = "ChromosomeCases Week of_2019-08-12.xlsx"
...
for index, row in enumerate(ws2.iter_rows()):
    ...
    try:
        if "S" in row[0].value or "CG" in row[0].value or "FL" in row[0].value:
            ...
            if datetime.date.today() == ws2.cell(row=index+2, column=5).value.date():
                #print("hi")
            ...
            if datetime.date.today() == ws2.cell(row=index+2, column=6).value.date():
                #print("no")
                ws.cell(column=3, row=new_line_num, value=row[0].value)
                ws.cell(column=4, row=new_line_num, value=row[1].value)
                ws.cell(column=5, row=new_line_num, value=row[2].value)
            ...
            if datetime.date.today() == ws2.cell(row=index+2, column=7).value.date():
                ws.cell(column=3, row=new_line_num, value=row[0].value)
                ws.cell(column=4, row=new_line_num, value=row[1].value)
                ws.cell(column=5, row=new_line_num, value=row[2].value)
                ws.cell(column=8, row=new_line_num, value=row[6].value)
                print(row[0].value + " " + row[6].value)
            if datetime.date.today() == ws2.cell(row=index+2, column=8).value.date():    
                ws.cell(column=3, row=new_line_num, value=row[0].value)
                ws.cell(column=4, row=new_line_num, value=row[1].value)
                ws.cell(column=5, row=new_line_num, value=row[2].value)
                ws.cell(column=9, row=new_line_num, value=row[7].value)
                print(row[0].value + " " + row[7].value)
            if datetime.date.today() == ws2.cell(row=index+2, column=9).value.date():
                ws.cell(column=3, row=new_line_num, value=row[0].value)
                ws.cell(column=4, row=new_line_num, value=row[1].value)
                ws.cell(column=5, row=new_line_num, value=row[2].value)
                ws.cell(column=10, row=new_line_num, value=row[8].value)
                print(row[0].value + " " + row[8].value)
            if datetime.date.today() == ws2.cell(row=index+2, column=10).value.date():
                ws.cell(column=3, row=new_line_num, value=row[0].value)
                ws.cell(column=4, row=new_line_num, value=row[1].value)
                ws.cell(column=5, row=new_line_num, value=row[2].value)
...
ws.oddHeader.left.text = 'GSL List_' + str(date.today() + timedelta(days=1)) 
...
wb.save('GSL Case List' + '_' + str(date.today() + timedelta(days=1)) + '.xlsx')
wb.close()
