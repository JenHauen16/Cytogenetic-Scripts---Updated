...
for index, row in enumerate(ws2.iter_rows()):
    ...
    nlwsn = wsn.max_row + 1
    try:
        if "S" in row[0].value or "CG" in row[0].value or "FL" in row[0].value or "SJS" in row[0].value:
...
ws.oddHeader.left.text = 'QC_SlideMaking' + str(date.today() + timedelta(days=1)) 
...
wb.save('QC_SlideMaking Week of' + '_' + str(last_monday) + '.xlsx')
wb.close()
